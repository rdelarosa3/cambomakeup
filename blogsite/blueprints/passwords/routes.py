...
def reset_request():
	...
	rendered_html = render_template('reset_request.html', title='Reset Password', form=form)
...
def reset_token(token):
	...
	if form.validate_on_submit():
		...
		return redirect(url_for('sessions.login'))
	rendered_html = render_template('reset_token.html', title='Reset Password', form=form)
	return html_minify(rendered_html)